...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (![3, 10, 25].includes(topNumber)) {
            return interaction.reply("Tu as juste à choisir un nombre valide : 3, 10 ou 25. À croire tu es Pagma qui lit l'heure sur sa rolex !");
        }
        ...
        try {
            ...
            // Calcul et tri
            const classement = personnages.map(personnage => {
                const stats = personnage.statistics_character[0];
                const totalStats = stats.vie + stats.endurance + stats.attaque + stats.defense + stats.vitesse;
                return {
                    nom: personnage.nom,
                    totalStats
                };
            ...
            console.log(error);
            await interaction.reply({ content: "Une erreur est survenue lors de l'affichage classement des personnages.", ephemeral: true });
        }
    }
};
