...
module.exports = {
    ...
    async execute(interaction) {
        const nom = interaction.options.getString('arme');
        ...
        try {
            ...
            if (!weapon) {
                return interaction.reply(`Si tu veux éditer une arme, commence par avoir un QI plus élévé qu'Azmog. ${capitalizedName} n'est même pas encore sur le jeu.`);
            }
            ...
            if (!statistique) {
                return await interaction.reply(`Si tu veux éditer une arme, commence par avoir un QI plus élévé qu'Azmog. Les stats de ${capitalizedName} n'existe pas encore donc tu ne peux pas l'éditer.`);
            } else {
            ...
            console.log(e);
            await interaction.reply({ content: "Une erreur est survenue lors de la modification de l'arme", ephemeral: true });
        }
    }
};